...
int main(int argc, char **argv)
{
    ...
    cb.debug = false;
    cb.noComm = false;
    cb.simTime = 1;                                    // Default simulation time
    std::string outputFileName = "data_strong_scaling.csv"; // Default file name
#ifdef _MPI_
    MPI_Init(&argc, &argv);
#endif
    ...
    // new added
    if (cb.niters == 0)
    { // If iterations not provided, calculate from simTime
        cb.niters = static_cast<int>(cb.simTime / dt);
        if (cb.niters <= 0)
        {
            std::cerr << "Error: Simulation time is too short for the calculated time step (dt = " << dt << ").\n";
            exit(-1);
        }
    }
    // End Initialization
